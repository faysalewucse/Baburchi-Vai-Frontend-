...
export default function Blog() {
  ...
  return (
    <div>
      <div className="bg-orange-100 h-24 flex justify-center items-center ">
        <ReactToPdf
          targetRef={ref}
          filename="blogs.pdf"
          scale={0.8}
        ...
        >
          {({ toPdf }) => (
            <FontAwesomeIcon
              onClick={toPdf}
              icon={faDownload}
              className="text-4xl text-secondary hover:text-secondary2 hover:scale-110 transition-all duration-200 cursor-pointer"
            />
          )}
        </ReactToPdf>
      </div>
      <div
        ref={ref}
        className="max-w-7xl mx-auto font-common mt-10 grid grid-cols-1 md:grid-cols-2 gap-5 text-2xl p-2 mb-10"
      >
        {/* Blog-1 */}
        <BlogCard
